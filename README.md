...
R2 требует **license key** от Difluid — это особенность их SDK (серверная аутентификация через `cloud-gateway-os.digitizefluid.com`). При добавлении R2 через HA будет форма с полем "License Key". Без ключа устройство работать не будет.
